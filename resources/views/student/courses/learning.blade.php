...
<body class="font-poppins text-[#0A090B]">
    <section id="content">
        ...
        <form action="{{ route('admin.learning.course_exam.answer_exam.store', ['course_exam' => $courseExam->id, 'question' => $questionexam->id]) }}" method="POST" class="learning flex flex-col gap-[50px] items-center mt-[50px] w-full pb-[30px]">
            @csrf
            <h1 class="w-[821px] font-bold text-[24px] leading-[35px] text-left">
                {{ $questionexam->question }}
            </h1>
            <div class="flex flex-col gap-[30px] max-w-[750px] w-full">

                @foreach($questionexam->answerexams as $answer)
