    ...
    <section id="content" class="flex">
        ...
        <div id="menu-content" class="flex flex-col w-full pb-[30px]">
                ...
                <div class="flex items-center gap-[30px]">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="flex flex-col text-right">
                            <p class="text-sm text-[#7F8190]">Hi,</p>
            ...
            <div class="course-list-container flex flex-col px-5 mt-[30px] gap-[30px]">
                ...
                @forelse($my_courses as $course_exam)
                <div class="flex justify-between pr-10 list-items flex-nowrap">
                    <div class="flex shrink-0 w-[300px]">
                        <div class="flex items-center gap-4">
                            <div class="flex w-16 h-16 overflow-hidden rounded-full shrink-0">
                                <img src="{{Storage::url($course_exam->cover)}}" class="object-cover" alt="thumbnail">
                            </div>
                            <div class="flex flex-col gap-[2px]">
                                <p class="text-lg font-bold">{{ $course_exam->name }}</p>
                                <p class="text-[#7F8190]">Beginners</p>
                            </div>
                        </div>
                    </div>
                    <div class="flex shrink-0 w-[150px] items-center justify-center">
                        <p class="font-semibold">{{ \Carbon\Carbon::parse($course_exam->created_at)->format('F j, Y') }}</p>
                    </div>

                    @if($course_exam->categoryexam->name == 'Tanaman')
                        <div class="flex shrink-0 w-[170px] items-center justify-center">
                            <p class="p-[8px_16px] rounded-full bg-[#D5EFFE] font-bold text-sm text-[#066DFE]">{{ $course_exam->categoryexam->name }}</p>
                        </div>
                        @elseif($course_exam->categoryexam->name == 'Teknik dan Pengolahan')
                        <div class="flex shrink-0 w-[170px] items-center justify-center">
                            <p class="p-[8px_16px] rounded-full bg-[#D5EFFE] font-bold text-sm text-[#066DFE]">{{ $course_exam->categoryexam->name }}</p>
                        </div>
                        @elseif($course_exam->categoryexam->name == 'Keuangan')
                        <div class="flex shrink-0 w-[170px] items-center justify-center">
                            <p class="p-[8px_16px] rounded-full bg-[#D5EFFE] font-bold text-sm text-[#066DFE]">{{ $course_exam->categoryexam->name }}</p>
                        </div>
                        @elseif($course_exam->categoryexam->name == 'SDM')
                        <div class="flex shrink-0 w-[170px] items-center justify-center">
                            <p class="p-[8px_16px] rounded-full bg-[#D5EFFE] font-bold text-sm text-[#066DFE]">{{ $course_exam->categoryexam->name }}</p>
                        </div>
                        @elseif($course_exam->categoryexam->name == 'Corporate Culture')
                        <div class="flex shrink-0 w-[170px] items-center justify-center">
                            <p class="p-[8px_16px] rounded-full bg-[#D5EFFE] font-bold text-sm text-[#066DFE]">{{ $course_exam->categoryexam->name }}</p>
                        </div>
                        @elseif($course_exam->categoryexam->name == 'Umum')
                        <div class="flex shrink-0 w-[170px] items-center justify-center">
                            <p class="p-[8px_16px] rounded-full bg-[#D5EFFE] font-bold text-sm text-[#066DFE]">{{ $course_exam->categoryexam->name }}</p>
                        </div>
                        @endif
                    ...
                    <div class="flex shrink-0 w-[120px] items-center">
                        @if($course_exam->nextQuestionId !== null)
                        <a href="{{ route('admin.learning.course_exam', ['course_exam' => $course_exam->id, 'question' => $course_exam->nextQuestionId]) }}" class="w-full h-[41px] p-[10px_20px] bg-[#6436F1] rounded-full font-bold text-sm text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#6436F14D] text-center">Start Test</a>
                        {{-- <a href="{{ route('admin.learning.course_exam') }}" class="w-full h-[41px] p-[10px_20px] bg-[#6436F1] rounded-full font-bold text-sm text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#6436F14D] text-center">Start Test</a> --}}
                        {{-- <button href="route('admin.learning.course_exam')" class="w-full h-[41px] p-[10px_20px] bg-[#6436F1] rounded-full font-bold text-sm text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#6436F14D] text-center">Start Test</button> --}}
                        @else
                        <a href="{{ route('admin.learning.rapport.course_exam', $course_exam) }}" class="w-full h-[41px] p-[10px_20px] bg-indigo-950 rounded-full font-bold text-sm text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#6436F14D] text-center">Lihat Hasil</a>
                        @endif
                    </div>
                </div>
                @empty
