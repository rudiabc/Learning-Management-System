...
<body class="font-poppins text-[#0A090B]">
    <section id="content" class="flex">
        ...
        <div id="menu-content" class="flex flex-col w-full pb-[30px]">
            ...
            <form method="POST" enctype="multipart/form-data" action="{{ route('admin.course_exams.update', $courseExam) }}" class="flex flex-col gap-[30px] w-[500px] mx-[70px] mt-10">
                @csrf
                @method('PUT')
                <div class="flex items-center gap-5">
                    ...
                    <div class="relative w-[100px] h-[100px] rounded-full overflow-hidden">
                        <div class="relative z-10 w-full h-full file-preview">
                            <img src="{{Storage::url($courseExam->cover)}}" class="object-cover w-full h-full thumbnail-icon" alt="thumbnail">
                        </div>
                ...
                <div class="flex flex-col gap-[10px]">
                    ...
                    <div class="flex items-center w-[500px] h-[52px] p-[14px_16px] rounded-full border border-[#EEEEEE] transition-all duration-300 focus-within:border-2 focus-within:border-[#0A090B]">
                        ...
                        <input value="{{ $courseExam->name }}" type="text" class="font-semibold placeholder:text-[#7F8190] placeholder:font-normal w-full outline-none" placeholder="Write your better course name" name="name" required>
                    </div>
                </div>
                <div class="group/category flex flex-col gap-[10px]">
                    ...
                    <div class="peer flex items-center p-[12px_16px] rounded-full border border-[#EEEEEE] transition-all duration-300 focus-within:border-2 focus-within:border-[#0A090B]">
                        ...
                        <select id="category" class="pl-1 font-semibold focus:outline-none w-full text-[#0A090B] invalid:text-[#7F8190] invalid:font-normal appearance-none bg-[url('{{asset('images/icons/arrow-down.svg')}}')] bg-no-repeat bg-right" name="category_exam_id" required>
                            <option value="{{ $courseExam->categoryexam->id }}" selected>{{ $courseExam->categoryexam->name }}</option>
                            @forelse($category_exams as $category_exam)
                                <option value="{{ $category_exam->id }}" class="font-semibold">
                                    {{ $category_exam->name }}
                                </option>
                            @empty
                            @endforelse
